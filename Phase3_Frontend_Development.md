...
#### 核心方案：SQLite + SQLModel
- **存储位置**：项目根目录下的 `data.db` 文件（一个标准的数据库文件）
- **特点**：轻量级、无需独立数据库服务、支持多用户并发
- **档案表**：存储用户孩子信息（昵称、出生日期、年级等）
- **历史表**：存储每个会话的聊天消息（role、content、时间戳）

#### 工作流程
```
用户创建档案 → FastAPI 接收请求 → SQLModel ORM 验证 
→ SQLite 执行 SQL 保存到 data.db 文件 
→ 服务重启后，数据从 data.db 恢复 → 用户可查看历史和档案
```

#### 生产路径
- **开发/小范围试用**（现在）：SQLite 文件存在服务器本地
- **大规模商业化**：迁移到 MySQL/PostgreSQL（仅需改一行配置，代码不变）
